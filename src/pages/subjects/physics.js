...
export default function PhysicsSubject() {
  ...
  useEffect(() => {
    ...
    // Загружаем данные физики из Firebase
    async function fetchPhysicsData() {
      try {
        setLoading(true);
        setError(null);
        
        // Получаем данные предмета физики из Firebase
        const data = await subjectsService.getSubjectById('2'); // Третий документ в коллекции subjects
        console.log('Physics data loaded from Firebase:', data);
        
        // Получаем документ регламента для физики (doc5)
        const docData = await documentsService.getDocumentById('doc5');
        console.log('Physics regulation document loaded from Firebase:', docData);
        
        if (data) {
          setPhysicsData(data);
          setRegulationDoc(docData);
        } else {
          setError('Данные по физике не найдены.');
        }
      } catch (err) {
        console.error("Ошибка при загрузке данных физики:", err);
  ...
  return (
    <Layout title="Физика">
      <div className="subject-header" style={{ background: 'linear-gradient(135deg, #dc3545 0%, #fd7e14 100%)' }}>
        <div className="container py-5 position-relative z-index-1">
          <div className="row align-items-center">
            <div className="col-lg-8">
              <h1 className="text-white display-4 fw-bold mb-3">{physicsData.title}</h1>
              <p className="text-white opacity-90 lead mb-4">{physicsData.shortDescription}</p>
            ...
            <div className="col-lg-4 d-none d-lg-block">
              <div className="subject-icon-wrapper">
                <div className="subject-icon-container">
                  <i className={`bi bi-${physicsData.icon} subject-icon-large`}></i>
        ...
        <div className="row g-4">
          <div className="col-lg-8">
            <section className="mb-5">
              <h2 className="section-title mb-4">О профиле «{physicsData.title}»</h2>
                ...
                <div className="col-md-6">
                  <div className="subject-info-card h-100">
                    ...
                    <p className="subject-info-text">
                      {physicsData.locations && physicsData.locations.length > 0 
                        ? physicsData.locations.join(', ') 
                ...
                <div className="d-flex">
                  ...
                  <div>
                    ...
                    <p className="subject-info-box-text">
                      Физика в рамках Арктической олимпиады охватывает все основные разделы: механику, термодинамику, 
                      электродинамику, оптику и квантовую физику. Особое внимание уделяется практическим задачам и 
                      экспериментальным заданиям.
                    </p>
                  </div>
                </div>
              </div>
                    ...
                    <div className="phase-info">
                      ...
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          ...
                          <div className="phase-value">
                            Все зарегистрированные школьники {physicsData.grades?.join(', ')} классов
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            <section className="mb-5">
              ...
              {physicsData.pastProblems && physicsData.pastProblems.length > 0 ? (
                <div className="past-problems-table">
                  {physicsData.pastProblems.map((yearData, index) => (
