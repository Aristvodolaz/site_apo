// Скрипт для рассылки email участникам по определенному предмету
// Отправляет письмо только тем, у кого в subjects есть указанный предмет
// Запуск: node src/scripts/sendEmailBySubject.cjs biology
// Или: node src/scripts/sendEmailBySubject.cjs biology 500 (начать с 500-й записи)
...
// Задержка между отправками (мс) - увеличена для избежания блокировки как спам
const DELAY_BETWEEN_EMAILS = 10000; // 10 секунд - увеличенная задержка для избежания блокировки
...
async function main() {
  console.log('\n' + '='.repeat(70));
  console.log('📧 РАССЫЛКА EMAIL ПО ПРЕДМЕТУ');
  ...
  try {
    ...
    // Получаем предмет из аргументов командной строки
    const subject = process.argv[2];
    if (!subject) {
      console.error('❌ ОШИБКА: Не указан предмет для рассылки');
      console.log('\nИспользование:');
      console.log('  node src/scripts/sendEmailBySubject.cjs <предмет> [стартовый_индекс]');
      console.log('\nПримеры:');
      console.log('  node src/scripts/sendEmailBySubject.cjs biology');
      console.log('  node src/scripts/sendEmailBySubject.cjs biology 500');
      console.log('  node src/scripts/sendEmailBySubject.cjs math');
      console.log('  node src/scripts/sendEmailBySubject.cjs physics');
      console.log('  node src/scripts/sendEmailBySubject.cjs chemistry');
      process.exit(1);
    }

    // Получаем стартовый индекс из аргументов командной строки
    const startIndex = process.argv[3] ? parseInt(process.argv[3], 10) : 0;
    ...
    console.log(`✓ Предмет для рассылки: ${subjectKey}`);
    ...
    // Фильтруем участников с email, participantId и нужным предметом
    const allParticipants = [];
    snapshot.forEach(doc => {
      const data = doc.data();
      if (data.email && data.participantId && data.subjects && Array.isArray(data.subjects)) {
        // Проверяем, есть ли нужный предмет в массиве subjects
        if (data.subjects.includes(subjectKey)) {
    ...
    console.log(`📊 Всего участников с предметом "${subjectKey}": ${allParticipants.length}`);
    
    if (allParticipants.length === 0) {
      console.error(`\n❌ ОШИБКА: Не найдено участников с предметом "${subjectKey}"`);
      console.log('\n💡 Проверьте правильность написания предмета:');
      console.log('   - biology (биология)');
      console.log('   - math (математика)');
      console.log('   - physics (физика)');
    ...
    participantsToSend.slice(0, 5).forEach((p, index) => {
      ...
      console.log(`      Предметы: ${p.subjects.join(', ')}`);
    });
    ...
    console.log(`   (участники с предметом "${subjectKey}")`);
    if (startIndex > 0) {
    ...
    for (let i = 0; i < participantsToSend.length; i++) {
      ...
      const actualIndex = startIndex + i + 1; // Реальный номер в списке участников с предметом
      ...
      if (result.success) {
        ...
        const isSpamError = result.error.includes('SPAM') || result.error.includes('554');
        const isRateLimitError = result.error.includes('Too many messages') || result.error.includes('450');
        
        if (isSpamError) {
          console.log(`   ⚠️  Заблокировано как спам (Yandex)`);
        } else if (isRateLimitError) {
          console.log(`   ⚠️  Лимит отправки (нужно подождать)`);
        } else {
        ...
        results.failed.push({
          email: participant.email,
          error: result.error,
          errorType: isSpamError ? 'spam' : isRateLimitError ? 'rate_limit' : 'other'
        });
        
        // Если получили ошибку спама, увеличиваем задержку
        if (isSpamError) {
          console.log(`   ⏸️  Увеличиваем задержку до 15 секунд...`);
          await delay(15000); // Дополнительная задержка после ошибки спама
        }
      }
    ...
    console.log('='.repeat(70));
    console.log(`\n📚 Предмет: ${subjectKey}`);
    console.log(`📊 Всего участников с предметом: ${allParticipants.length}`);
    console.log(`📤 Участников к отправке: ${participantsToSend.length}`);
    console.log(`✅ Успешно отправлено: ${results.success.length} писем`);
    ...
    const reportPath = path.join(__dirname, '..', '..', `email-report-${subjectKey}-${Date.now()}.json`);
    const report = {
      timestamp: new Date().toISOString(),
      subject: subjectKey,
      startIndex: startIndex,
      totalWithSubject: allParticipants.length,
