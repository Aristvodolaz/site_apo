// Скрипт для МАССОВОЙ рассылки email всем зарегистрированным участникам
// Отправляет письмо с напоминанием о номере регистрации и информацией о Telegram боте
// Запуск: node src/scripts/sendMassEmail.cjs
...
// Задержка между отправками (мс) - чтобы не превысить лимиты SMTP
const DELAY_BETWEEN_EMAILS = 2000; // 2 секунды
...
async function main() {
  console.log('\n' + '='.repeat(70));
  console.log('📧 МАССОВАЯ РАССЫЛКА EMAIL УЧАСТНИКАМ ОЛИМПИАДЫ');
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      if (data.email && data.participantId) {
        ...
        skipped.push({
          id: doc.id,
          reason: !data.email ? 'нет email' : 'нет participantId'
        });
      }
    });

    console.log(`\n📊 СТАТИСТИКА:`);
    console.log(`   ✅ Готовы к рассылке: ${participants.length} участников`);
    console.log(`   ⚠️  Пропущено: ${skipped.length} записей`);
    
    if (skipped.length > 0) {
      console.log('\n⚠️  Пропущенные записи:');
      skipped.forEach((item, index) => {
        console.log(`   ${index + 1}. ID: ${item.id} - причина: ${item.reason}`);
      });
    }
    ...
    participants.slice(0, 5).forEach((p, index) => {
      console.log(`   ${index + 1}. ${p.firstName} ${p.lastName} - ${p.email} (${p.participantId})`);
    });
    if (participants.length > 5) {
      console.log(`   ... и еще ${participants.length - 5} участников`);
    }

    // Запрашиваем подтверждение
    console.log('\n' + '='.repeat(70));
    console.log('⚠️  ВНИМАНИЕ! Вы собираетесь отправить письма на ' + participants.length + ' адресов');
    console.log('='.repeat(70));
    ...
    console.log(`⏱️  Примерное время выполнения: ~${Math.ceil((participants.length * DELAY_BETWEEN_EMAILS) / 1000 / 60)} минут\n`);
    ...
    for (let i = 0; i < participants.length; i++) {
      ...
      console.log(`[${i + 1}/${participants.length}] (${progress}%) Отправка: ${participant.email}...`);
